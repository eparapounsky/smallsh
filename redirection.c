...
void handle_input_redirection(struct user_command *current_command)
{
	...
	else if (current_command->is_background_process)
	{ // background process with no input redirection specified
...
void handle_output_redirection(struct user_command *current_command)
{
	...
	{ // background process with no output redirection specified
		int dev_null_fd = open("/dev/null", O_RDONLY);
